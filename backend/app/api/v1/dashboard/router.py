...
async def read_user_quick_stats(
    current_user: CurrentUser, dashboard_service: DashboardSvc
...
async def read_admin_stats(dashboard_service: DashboardSvc) -> list[AdminQuickStatResponse]:
...
async def read_admin_category_stats(dashboard_service: DashboardSvc) -> list[CategoryStatResponse]:
...
async def read_admin_borrow_overview(
    dashboard_service: DashboardSvc, period: str = Query("3m", pattern="^(3m|6m|12m)$")
) -> BorrowOverviewResponse:
