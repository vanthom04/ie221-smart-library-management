...
function ToastItem({ toast, index, onRemove, isTop }: ToastItemProps) {
  ...
  const icons: Record<ToastType, React.ReactNode> = {
    ...
    warning: <AlertTriangleIcon className="size-5 text-amber-500" />,
    info: <InfoIcon className="size-5 text-blue-500" />,
    default: <BellIcon className="size-5 text-muted-foreground" />
  }
  ...
  return (
    <motion.div
      ...
      className={cn(
        "pointer-events-auto relative max-w-105 min-w-[320px] overflow-hidden rounded-lg border border-l-4 bg-card p-4 shadow-lg",
        borderColors[type]
      )}
    >
      ...
      {duration > 0 && (
        <motion.div
          ...
          className={cn(
            "absolute right-0 bottom-0 left-0 h-1 origin-left",
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className={cn(
            "fixed bottom-8 left-1/2 z-50 -translate-x-1/2 rounded-full px-6 py-3 text-sm font-medium text-background shadow-lg dark:text-white",
            bgColors[type]
          )}
        >
          {message}
        ...
        >
          <div className="flex items-center gap-4 px-4 py-3">
            ...
            <button
              ...
              className="rounded-md bg-primary px-3 py-1 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90"
            >
              Undo
            </button>
          </div>
