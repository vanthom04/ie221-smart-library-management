      ...
      <CardContent className="flex flex-1 flex-col px-0">
        ...
        >
          ...
          <div className="my-auto flex flex-col items-center justify-center">
            ...
            <div className="leading-tight">
              {book.cover.titleLines.map((line) => (
                <p
                  key={`${book.id}-${line}`}
                  className="text-xs font-black tracking-tight uppercase sm:text-sm"
                >
                  {line}
                </p>
              ))}
        ...
        <div className="flex flex-1 flex-col justify-between p-4">
          <div>
            ...
            <h4 className="mt-2 line-clamp-2 text-sm leading-snug font-bold" title={book.title}>
              <button
                type="button"
                onClick={() => onSelectBook(book)}
                className="cursor-pointer text-left font-bold text-foreground transition-colors hover:text-primary"
              >
                {book.title}
              </button>
            </h4>
          ...
          <div className="mt-4 border-t border-border/60 pt-3">
            <div className="flex items-center justify-between gap-2">
              ...
              >
                ...
                />
                {isAvailable ? `Còn ${book.availableCount}/${book.totalCount} cuốn` : "Hết sẵn có"}
              </span>
